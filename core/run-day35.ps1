...
# day35 = 개입 효과 측정(개입 후 상태 / 개입 추천 상태 기준)
$expectedMap = [ordered]@{
  blank = [ordered]@{
    level  = "WARNING"
...
function Save-CoachArtifact {
  ...
  $outFile = Join-Path $artifactsDir ("coach.day35.{0}.user{1}.{2}.json" -f $tag, $userId, $stamp)
...
function Seed-Recovery {
  ...
  Add-Events -userId $userId -justOpen 1 -quiz 1 -wrong 2 -daysAgo 2
  Add-Events -userId $userId -justOpen 1 -quiz 1 -wrong 1 -wrongDone 2 -daysAgo 1
...
if($mode -in @("seed","all")){
  Write-Banner "DAY35 INTERVENTION EFFECT MEASUREMENT"
...
if($mode -in @("verify","all")){
  Write-Banner "DAY35 COACH ARTIFACT SAVE"
