...
function Pick-Latest([System.IO.FileInfo[]]$files,[string]$tag,[long]$uid){
  $matched = @(
    $files |
    Where-Object { $_.Name -match "^coach\.day33\.(blank|comeback|steady|wrongheavy|recovery|anomaly)\.user\d+\.\d{8}-\d{9}\.json$" } |
    Where-Object { $_.Name -match ("^coach\.day33\.{0}\.user{1}\.\d{{8}}-\d{{9}}\.json$" -f [regex]::Escape($tag), $uid) } |
...
if($rows.Length -eq 0){
  Write-Host ""
  Write-Host "[FAIL] No valid day33 data found." -ForegroundColor Red
  exit 1
}

Write-Host ""
Write-Host "==== DAY33 LONG-TERM PATTERN SUMMARY ====" -ForegroundColor Cyan
$rows | Select-Object `
  userId, tag,
  total_events, opens, quiz, wrong, wrong_done, active_days,
  today_events, today_opens, today_quiz, today_wrong, today_wrong_done,
...
Write-Host ""
Write-Host "==== DAY33 DATE RANGE CHECK ====" -ForegroundColor Cyan
$rows | Select-Object userId, tag, first_day, last_day, today_first_day, today_last_day | Format-Table -AutoSize

Write-Host ""
Write-Host "==== DAY33 ASSERTIONS ====" -ForegroundColor Yellow
...
if($comeback -and $steady){
  if((To-Int $steady.active_days) -le (To-Int $comeback.active_days)){
    Write-Host "[WARN] steady active_days <= comeback active_days" -ForegroundColor Yellow
    $warned = $true
  } else {
    Write-Host "[OK]   steady active_days > comeback active_days" -ForegroundColor Green
  }
}

if($wrongH -and $recovery){
  if((To-Int $wrongH.today_wrong) -le (To-Int $recovery.today_wrong)){
    Write-Host "[WARN] wrongheavy today_wrong <= recovery today_wrong" -ForegroundColor Yellow
    $warned = $true
  } else {
  ...
  if((To-Int $recovery.today_wrong_done) -le (To-Int $wrongH.today_wrong_done)){
    Write-Host "[WARN] recovery today_wrong_done <= wrongheavy today_wrong_done" -ForegroundColor Yellow
    $warned = $true
  } else {
...
if($anomaly){
  if((To-Int $anomaly.today_opens) -lt 6){
    Write-Host "[WARN] anomaly today_opens expected >= 6" -ForegroundColor Yellow
    $warned = $true
  } else {
    Write-Host "[OK]   anomaly today_opens >= 6" -ForegroundColor Green
  }

  if((To-Int $anomaly.today_quiz) -ne 0){
    Write-Host "[FAIL] anomaly today_quiz should be 0 but was $($anomaly.today_quiz)" -ForegroundColor Red
    $failed = $true
  } else {
...
if($failed){
  Write-Host "[FAIL] verify-day33 completed with assertion failures." -ForegroundColor Red
  exit 1
}

if($warned){
  Write-Host "[WARN] verify-day33 completed with warnings." -ForegroundColor Yellow
  exit 0
}

Write-Host "[OK] verify-day33 completed with no failures." -ForegroundColor Green
exit 0