...
function Curl-PostJson([string]$url,[string]$json){
  $tmp = Join-Path $env:TEMP ("req.day28.{0}.json" -f (Get-Random))
...
function Db([string]$sql){
  $tmp = Join-Path $env:TEMP ("sql.day28.{0}.sql" -f (Get-Random))
  $containerTmp = "/tmp/day28.$([guid]::NewGuid().ToString('N')).sql"

  $cleanSql = $sql.Trim()

  $headLen = [Math]::Min(5, $cleanSql.Length)
  $codes = ($cleanSql.Substring(0, $headLen).ToCharArray() | ForEach-Object { [int][char]$_ }) -join ","
  Write-Host "SQL HEAD CODES = $codes" -ForegroundColor Yellow

  $textLen = [Math]::Min(20, $cleanSql.Length)
  Write-Host ("SQL HEAD TEXT = [{0}]" -f $cleanSql.Substring(0, $textLen)) -ForegroundColor DarkYellow
...
function Save-Artifact([long]$uid,[string]$tag,[object]$payload){
  ...
  $file = Join-Path $dir ("coach.day28.{0}.user{1}.{2}.json" -f $tag,$uid,(Get-Date -Format "yyyyMMdd-HHmmssfff"))
...
function Print-Summary([long]$uid,[string]$tag,[object]$coach,[object]$raw,[object]$expected){
  $p = $coach.prediction
  ...
  Write-Host (" expect:   total={0} open={1} quiz={2} wrong={3} wrongDone={4} activeDays={5}" -f `
    $expected.total_events,$expected.opens,$expected.quiz,$expected.wrong,$expected.wrong_done,$expected.active_days)
}
...
function New-Expected(
  ...
  [pscustomobject]@{
    total_events = $open + $quiz + $wrong + $wrongDone
    opens        = $open
    quiz         = $quiz
    wrong        = $wrong
...
function Run-Phase([long]$uid,[string]$tag,[object]$expected,[scriptblock]$seedFn){
  Reset-History $uid
  & $seedFn
  ...
  Print-Summary $uid $tag $coach $raw $expected
...
foreach($uid in $userIds){
  ...
  Write-Host "DAY28 USER $uid" -ForegroundColor Yellow
  Write-Host "==============================" -ForegroundColor Yellow

  # qualityA : 분산형 꾸준 학습 + 일부 오답 + 일부 회복
  Run-Phase $uid "qualityA" (New-Expected 3 4 2 1 3) {
    Seed-Phase $uid 1 1 0 0 2
    Seed-Phase $uid 1 2 1 0 1
    Seed-Phase $uid 1 1 1 1 0
  }

  # qualityB : 오늘만 몰빵
  Run-Phase $uid "qualityB" (New-Expected 2 4 2 1 1) {
    Seed-Phase $uid 2 4 2 1 0
  }

  # qualityC : 과거만 있고 오늘 없음
  Run-Phase $uid "qualityC" (New-Expected 1 2 1 0 1) {
    Seed-Phase $uid 1 2 1 0 3
  }

  # qualityD : 회복 비중이 큰 케이스
  Run-Phase $uid "qualityD" (New-Expected 3 3 1 3 3) {
    Seed-Phase $uid 1 2 1 0 2
    Seed-Phase $uid 1 0 0 1 1
    Seed-Phase $uid 1 1 0 2 0
  }

  # qualityE : 열기만 많은 저품질 활동
  Run-Phase $uid "qualityE" (New-Expected 4 0 0 0 1) {
    Seed-Phase $uid 4 0 0 0 0
  }
}

Write-Host ""
Write-Host "DAY28 TUNED DONE" -ForegroundColor Green