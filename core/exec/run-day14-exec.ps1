# C:\dev\loosegoose\goosage-scripts\core\run-day14-exec.ps1
param(
  ...
  # 유저별 preset 매핑 (여기를 Day14 기준으로 바꿔라)
...
function Curl-PostJson([string]$url, [string]$jsonBody, [hashtable]$headers = @{}) {
  $tmp = Join-Path $env:TEMP ("req.day14.{0}.{1}.json" -f $PID, (Get-Random))
...
function Save-Coach([long]$uid, [string]$phaseTag, [string]$preset, [string]$json) {
  ...
  $outFile = Join-Path $outDir ("coach.day14.{0}.u{1}.{2}.{3}.json" -f $phaseTag, $uid, $preset, $ts)
