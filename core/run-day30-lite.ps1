...
function Get-Coach([long]$uid){
  $text = Curl-Text "$base/internal/study/coach?userId=$uid" @{ "X-INTERNAL-KEY" = $internalKey }

  if([string]::IsNullOrWhiteSpace($text)){
    Fail "coach response is empty. base=$base userId=$uid"
  }
  ...
  try {
    ...
    Fail "coach response is not valid JSON. base=$base userId=$uid response=[$text]"
  }
}

function Parse-RawCols($raw){
  $cols = @()
  if($null -ne $raw){
    $cols = @($raw -split '\s+')
  }
