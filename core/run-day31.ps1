...
function Curl-PostJson([string]$url,[string]$jsonBody,[hashtable]$headers=@{}){
  $tmp = Join-Path $env:TEMP ("req.day31.{0}.{1}.json" -f $PID,(Get-Random))
...
function Db([string]$sql){
  $tmp = Join-Path $env:TEMP ("day31_sql_{0}_{1}.sql" -f $PID,(Get-Random))
  Write-Utf8NoBom $tmp $sql

  try {
    docker cp $tmp "${mysqlContainer}:/tmp/day31.sql" | Out-Null
    docker exec $mysqlContainer sh -lc "mysql -N -u$dbUser -p$dbPass $dbName < /tmp/day31.sql"
  }
  finally {
    Remove-Item $tmp -Force -ErrorAction SilentlyContinue
    docker exec $mysqlContainer sh -lc "rm -f /tmp/day31.sql" | Out-Null
  }
}
...
function Save-Coach([long]$uid,[string]$tag,[object]$obj){
  ...
  $file = Join-Path $dir ("coach.day31.{0}.user{1}.{2}.json" -f $tag,$uid,$ts)
...
# Day31 = persona 자동 시뮬레이션 / 일관성 검증
# 같은 시나리오를 여러 유저에 반복 삽입해서 결과 안정성을 본다.
$scenarioTemplates = @(
  [pscustomobject]@{
    tag    = "spamopen"
    title  = "SPAMOPEN"
    phases = @(
      @{ open = 10; quiz = 0; wrong = 0; wrongDone = 0; daysAgo = 1 },
  ...
  [pscustomobject]@{
    ...
    title  = "QUIZONLY"
    phases = @(
      @{ open = 0; quiz = 4; wrong = 0; wrongDone = 0; daysAgo = 1 },
      @{ open = 0; quiz = 6; wrong = 0; wrongDone = 0; daysAgo = 0 }
    )
  },
  [pscustomobject]@{
    tag    = "wrongheavy"
    title  = "WRONGHEAVY"
    phases = @(
      @{ open = 1; quiz = 1; wrong = 4; wrongDone = 0; daysAgo = 1 },
      @{ open = 1; quiz = 1; wrong = 6; wrongDone = 1; daysAgo = 0 }
    )
  },
  [pscustomobject]@{
    tag    = "recoverybias"
    ...
    phases = @(
      ...
      @{ open = 1; quiz = 1; wrong = 1; wrongDone = 10; daysAgo = 0 }
    )
  },
  [pscustomobject]@{
    tag    = "balanced"
    title  = "BALANCED"
    phases = @(
      @{ open = 1; quiz = 2; wrong = 1; wrongDone = 1; daysAgo = 1 },
      @{ open = 1; quiz = 2; wrong = 1; wrongDone = 1; daysAgo = 0 }
    )
  },
  [pscustomobject]@{
    tag    = "lowqualitymix"
    title  = "LOWQUALITYMIX"
    phases = @(
      @{ open = 5; quiz = 1; wrong = 0; wrongDone = 0; daysAgo = 1 },
      @{ open = 6; quiz = 1; wrong = 1; wrongDone = 0; daysAgo = 0 }
    )
  }
)
...
foreach($uid in $users){
  ...
  Warn "=============================="
  Warn "DAY31 PERSONA LOOP userId=$uid"
  Warn "=============================="

  foreach($sc in $scenarioTemplates){

    $tag   = [string]$sc.tag
    $title = [string]$sc.title

    Warn "------------------------------"
    Warn "DAY31 $title userId=$uid"
    Warn "------------------------------"
