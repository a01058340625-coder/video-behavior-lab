...
function Post-InternalEvent {
  ...
  if ($null -eq $resp -or $resp.success -ne $true) {
    throw "internal event insert ½ÇÆÐ userId=$userId type=$type"
  }
}
...
function Save-CoachArtifact {
  ...
  $outFile = Join-Path $artifactsDir ("coach.day40.{0}.user{1}.{2}.json" -f $tag, $userId, $stamp)
...
function Seed-Collapse {
  param([long]$userId)
  Write-Host "[SCENARIO] collapse user=$userId" -ForegroundColor Yellow
}

function Seed-Restart {
  param([long]$userId)
  Write-Host "[SCENARIO] restart user=$userId" -ForegroundColor Yellow
...
function Seed-Stable {
  param([long]$userId)
  Write-Host "[SCENARIO] stable user=$userId" -ForegroundColor Yellow
  4..0 | ForEach-Object {
    $d = $_
...
function Seed-OpenOnly {
  param([long]$userId)
  Write-Host "[SCENARIO] openonly user=$userId" -ForegroundColor Yellow
  Add-Events -userId $userId -justOpen 6 -daysAgo 0
}

function Seed-RecoverySafe {
  param([long]$userId)
  Write-Host "[SCENARIO] recoverysafe user=$userId" -ForegroundColor Yellow
  Add-Events -userId $userId -justOpen 1 -quiz 2 -wrong 2 -wrongDone 2 -daysAgo 3
  Add-Events -userId $userId -justOpen 1 -quiz 2 -wrong 1 -wrongDone 3 -daysAgo 2
  Add-Events -userId $userId -justOpen 1 -quiz 2 -wrong 1 -wrongDone 3 -daysAgo 1
  Add-Events -userId $userId -justOpen 1 -quiz 2 -wrong 0 -wrongDone 2 -daysAgo 0
}

if($mode -in @("seed","all")){
  Write-Banner "DAY40 DECISION ENGINE FINAL HARDENING"

  Reset-Users -userIds ($scenarioMap.Values | ForEach-Object { [long]$_ })

  Seed-Collapse     -userId ([long]$scenarioMap.collapse)
  Seed-Restart      -userId ([long]$scenarioMap.restart)
  Seed-Stable       -userId ([long]$scenarioMap.stable)
  Seed-WrongHeavy   -userId ([long]$scenarioMap.wrongheavy)
  Seed-Recovery     -userId ([long]$scenarioMap.recovery)
  Seed-OpenOnly     -userId ([long]$scenarioMap.openonly)
...
if($mode -in @("verify","all")){
  Write-Banner "DAY40 COACH ARTIFACT SAVE"
